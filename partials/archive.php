<?php  if(!defined('BASEPATH')) exit('No direct script access allowed'); ?>


<h2 class="font-bold text-heading text-lg lg:text-2xl">Arsip Konten Situs Web <?= $desa['nama_desa'] ?></h2>
<div class="divider"></div>
<div class="overflow-x-auto main-content">
  <?php if(count($farsip)>0): ?>
		<table class="table table-compact w-full">
      <thead>
        <tr>
          <td width="3%"><b>No.</b></td>
          <td width="20%"><b>Tanggal Artikel</b></td>
          <td><b>Judul Artikel</b></td>
          <td width="20%"><b>Penulis</b></td>
        </tr>
      </thead>
      <tbody>
      <?php foreach($farsip AS $data): ?>
        <tr class="hover">
          <td>
            <?= $data["no"]?>
          </td>
          <td>
            <?= tgl_indo($data["tgl_upload"])?>
          </td>
          <td>
            <a class="link link-hover with-underline" href="<?= site_url('artikel/'.buat_slug($data))?>"><?= $data["judul"]?></a>
          </td>
          <td>
            <?= $data["owner"]?>
          </td>
        </tr>
      <?php endforeach; ?>
      </tbody>
    </table>
  <?php else: ?>
    Belum ada arsip konten web.
  <?php endif; ?>
</div>
